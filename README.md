...
- **Physical model strip** (always visible, between stage and keyboard): Rayleigh damping law (αM/βK per the paper's C = αM + βK), boundary support, FEM-resolution morph (4³→8³ baked tables), body morphing with target dropdown, material physics rescale, ECO toggle + budget — no modals, everything on one 1440×990 screen
