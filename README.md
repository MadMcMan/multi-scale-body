...
Beyond paper 47's Dirac strike, the Bow knob turns held notes into bowed swells: a velocity-driven stick-slip friction bridge (constant bow speed, static/dynamic friction hysteresis, stick window) excites the *same* modal bank at the strike position, injected through the sustained-drive normalizer. Bowed sustain is per-voice leveled to the Bowl reference (`bowScale`, boost-only ≤6×, signed static-mix metric): measured, the quasi-static bridge never enters stick-slip on sparse bodies, so un-levelled constant-force loudness followed modal Q and Glass/Chime/Celesta whispered at C4 while Bowl roared (150× spread) — after leveling every factory body speaks and the spread is bounded. Switching the body preset mid-hold re-bakes sounding voices at their stored strike point (tables were previously frozen at noteOn, so a held bowed note kept droning the old body after a preset change). Velocities, decay, band trims, felt and the limiter all apply as for struck notes; note-off (or disc release) lifts the bow and the body rings free. Default (bow off) keeps the classic mallet path bit-identical.
